...
import nengo
import numpy as np
from sparserl.utils import sparsity_to_x_intercept
...
class ActorCritic(object):
    ...
    def __init__(self, representation, rule, n_neurons=None, sparsity=None, neuron_type=nengo.RectifiedLinear(),
                 report_spikes = False, **ensemble_args):
        ...
        with self.model:
            ...
            ## if we are using a neuron ensemble
                ...
                ##connect the rest of the action, reward and reset information to the rule node
            ...
            ## Probes
            if self.report_spikes == True:
                self.p_spikes = nengo.Probe(self.ensemble.neurons)
    ...
    def step(self, state, update_action, reward, reset=False, report = False):
        ...
        if report == True:
            ## Fetch probe data
            self.spikes = self.sim.data[self.p_spikes]
            
            return self.state_value, self.action_values, self.spikes
        else:
